...
if strcmp(expinfo.ListGen,'AsIs') % Take Trial Configurations as they are
    TrialMatrix = expinfo.TrialConfigurations(1:nTrials,:)
elseif strcmp(expinfo.ListGen,'FullRandom') % Fully random trial list
    TrialMatrix = expinfo.TrialConfigurations(randsample(size(expinfo.TrialConfigurations,1),nTrials),:);
    Trial(nTrials).condition = TrialMatrix(nTrials,3);
    
    
    %You can follow this as a guide
    trialsSelected = 0;
    maxIter = 1000; % Specify maximum number of allowed iterations
    while trialsSelected ~= nTrials
        trialsSelected = 0;
        TrialMatrix = zeros(nTrials,size(expinfo.TrialConfigurations, 2));
        SampleMatrix = expinfo.TrialConfigurations;
        for trial = 1:nTrials
